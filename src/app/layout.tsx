...
export default function RootLayout({ children }: LayoutProps<"/">) {
  return (
    ...
    >
      {/* המעטפת כולה (body) היא בגובה קבוע (svh) ולא גוללת בעצמה —
          הגלילה קורית רק בתוך PullToRefresh (ראו הרכיב עצמו ו-
          app/(app)/layout.tsx). זה בא אחרי כמה סיבובים של תיקוני-CSS
          חלקיים לבאג שספארי באייפון (PWA שמור למסך הבית) גורם ל-nav
          קבוע (position:fixed) "לקפוץ" או אפילו להיתלש לגמרי תוך כדי
          גלילה — קודם ניסינו dvh→svh על גובה הדף, ואז transform-gpu
          על ה-nav עצמו, ושני הניסיונות ההם רק הזיזו את התקלה למקום
          אחר. הפתרון האמיתי הוא לא לתת ל-nav להיות position:fixed
          בכלל: כשרק אזור תוכן אחד גולל (לא כל הדף), הכותרת וה-nav
