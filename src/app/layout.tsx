...
export default function RootLayout({ children }: LayoutProps<"/">) {
  return (
    <html
      lang="he"
      dir="rtl"
      className={`${rubik.variable} ${assistant.variable} h-svh antialiased`}
    >
      {/* svh, לא dvh — ניסינו קודם dvh וזה יצר באג אחר: dvh משתנה
          בזמן אמת בכל פעם שסרגל הכתובת של הדפדפן מתחבא/מופיע תוך כדי
          גלילה (בנייד), וה-nav הקבוע (position:fixed; bottom:0) שיושב
          בתוך עטיפה שגובהה תלוי ב-dvh "קפץ" בדיוק בזמן הגלילה, וחוזר
          למקום כשגוללים בחזרה למעלה — בדיוק מה ששיר תפסה בפועל.
          svh הוא הגובה הקטן ביותר האפשרי (כשסרגל הדפדפן כן מוצג) ולא
          משתנה כלל תוך כדי גלילה — יציב לגמרי. עדיין פותר את הבעיה
          המקורית (h-full/min-h-full לא תמיד מתאים לגובה הנראה בפועל,
          מה שגרם ל-nav לקפוץ לאמצע העמוד כשהתוכן קצר מהמסך), רק בלי
          התנודה של dvh. המחיר: כשסרגל הדפדפן מוסתר יש קצת שטח לבן
          נוסף בתחתית שלא מנוצל — פשרה סבירה בהרבה מ-nav שקופץ. */}
