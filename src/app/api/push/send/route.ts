 ...
 * morning: משלוש שעות לפני תחילת המפגש ועד שלוש שעות אחריו — טווח רחב
 *   בכוונה, כדי לספוג פערים של שעות בין טיקים בפועל (חלון "עד תחילתו
 *   בלבד" השאיר לרוב טיק אפשרי יחיד, ואם הוא דולג — אבודה לצמיתות).
 *   "בערך שעה לפני", לא בדיוק שעה — זה תואם למה שהוסכם: עדיף תזכורת
 *   שמגיעה, גם אם לא בדיוק בזמן, על פני תזכורת שלא מגיעה בכלל.
 *
...
export async function POST(request: NextRequest) {
  ...
  // "בערך שעה לפני" המפגש — בפועל חלון רחב, 3 שעות לפני **ועד 3 שעות
  // אחרי** תחילתו (לא רק עד תחילתו) — כדי לספוג פערים בין טיקים
  // בפועל. חלון "עד תחילתו" בלבד נתן בפועל טיק אפשרי יחיד לרוב
  // המפגשים (למשל מפגש 07:00 עם טיקים כל שעתיים ב-05:07/07:07 —
  // רק 05:07 נכנס), ואם דווקא הוא דולג, התזכורת אבודה לצמיתות. ראו
  // ההערה למעלה: עדיפה תזכורת שמגיעה קצת לא בדיוק בזמן, על פני
  // תזכורת שלא מגיעה בכלל.
  const REMINDER_WINDOW_BEFORE_MS = 3 * 3600_000;
  const REMINDER_WINDOW_AFTER_MS = 3 * 3600_000;
  const { data: soonCandidates } = await db
    .from("events")
    .select("id, club_id, starts_at, location_name")
    .gte("starts_at", new Date(now.getTime() - REMINDER_WINDOW_AFTER_MS).toISOString())
    .lte("starts_at", new Date(now.getTime() + REMINDER_WINDOW_BEFORE_MS).toISOString());
