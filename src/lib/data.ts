...
/**
 * עד שני ניסיונות חוזרים (שלושה סה"כ) לשאילתה שנכשלה — למשל טלפון
 ...
 * חלק מהקהילה" — כולל מנהלת. ניסיון חוזר אחד בלבד לא הספיק בפועל
 * (שיר דיווחה על המסך הזה שוב, ורענון פתר מיד — בדיוק כמו כשל רגעי
 * שהניסיון הראשון עוד לא הספיק לספוג); השהיה קצרה בין ניסיונות נותנת
 * לרשת/לחיבור רגע להתאושש במקום לפגוע בו שוב מיד.
 */
async function withRetry<T extends { error: unknown }>(
  run: () => PromiseLike<T>,
): Promise<T> {
  let last = await run();
  for (let attempt = 0; attempt < 2 && last.error; attempt++) {
